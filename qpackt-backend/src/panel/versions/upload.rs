...
async fn serve_request(mut payload: Multipart, config: Data<Config>, dao: Data<Dao>, versions: Data<Versions>) -> Result<VersionName> {
    let field = payload
        .try_next()
        .await
        .map_err(|e| QpacktError::MultipartUploadError(e.to_string()))?
        .ok_or_else(|| QpacktError::MultipartUploadError("No `next` field in multipart request".into()))?;
...
async fn wait_for_content(mut field: Field, target: &Path) -> Result<PathBuf> {
    ...
    while let Some(chunk) = field.next().await {
        let data = chunk.map_err(|e| QpacktError::MultipartUploadError(e.to_string()))?;
...
async fn unzip_and_register(zip_path: &Path, target: &Path, name: VersionName, app_run_dir: &Path, dao: &Dao) -> Result<Version> {
    ...
    let web_root = web_root
        ...
        .map_err(|e| QpacktError::UnableToProcessSite(format!("unable to strip site prefix: {}", e)))?;
...
fn unzip_site(path: &Path, target: &Path) -> Result<PathBuf> {
    let file = File::open(path).map_err(|e| QpacktError::UnableToProcessSite(format!("unable to open site file: {}", e)))?;
    let mut archive = zip::ZipArchive::new(file).map_err(|e| QpacktError::UnableToProcessSite(format!("unable to read zip: {}", e)))?;
    archive.extract(target).map_err(|e| QpacktError::UnableToProcessSite(format!("unable to unzip: {}", e)))?;
    remove_file(path).map_err(|e| QpacktError::UnableToProcessSite(format!("unable to unzip: {}", e)))?;
